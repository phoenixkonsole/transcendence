...
#include "guiutil.h"
#include "qt/transcendence/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(TELOSGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zTELOS anonymizes your TELOS by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zTELOS or convert back to TELOS"));
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zTELOS");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zTELOS");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zTELOS");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zTELOS");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zTELOS");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zTELOS");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zTELOS");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zTELOS");
    ...
    // Only Convert to TELOS enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzTELOS->setTitleClassAndText("btn-title-grey", "Total 0 zTELOS");
    ui->btnTotalzTELOS->setSubTitleClassAndText("text-subtitle", "Show denominations of zTELOS owned.");
    ui->btnTotalzTELOS->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select TELOS outputs to mint into zTELOS.");
    ...
    connect(ui->btnTotalzTELOS, SIGNAL(clicked()), this, SLOT(onTotalZTelosClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zTELOS");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of TELOS to mint into zTELOS"));
    }else{
        btnText = tr("Convert back to TELOS");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zTELOS to convert back into TELOS"));
    }
...
void PrivacyWidget::onTotalZTelosClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzTELOS->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzTELOS->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zTELOS is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zTELOS, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zTELOS").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zTELOS minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZTelos(
        ...
        // Spend succeed
        inform(tr("zTELOS converted back to TELOS"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzTELOS->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
