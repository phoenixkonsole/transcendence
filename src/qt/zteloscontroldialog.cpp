...
void ZTelosControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZTelosControlWidgetItem *itemMint = new CZTelosControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->ztelosTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->ztelosTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zTELOS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zTELOS seed used to mint this zTELOS is not the same as currently hold in the wallet";
            else
