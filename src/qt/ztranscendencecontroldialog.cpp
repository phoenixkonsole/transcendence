...
#include "ui_ztranscendencecontroldialog.h"
...
std::list<std::string> ZTranscendenceControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTranscendenceControlDialog::listMints;

ZTranscendenceControlDialog::ZTranscendenceControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTranscendenceControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTranscendenceControlDialog::~ZTranscendenceControlDialog()
{
    delete ui;
}

void ZTranscendenceControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTranscendenceControlDialog::updateList()
{
...
void ZTranscendenceControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTranscendenceControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTranscendence_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTranscendenceControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTranscendenceControlDialog::GetSelectedMints()
{
...
void ZTranscendenceControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
