...
#include "ztranscendencecontroldialog.h"
#include "spork.h"
...
void PrivacyDialog::on_pushButtonZTranscendenceControl_clicked()
{
    ZTranscendenceControlDialog* zTELOSControl = new ZTranscendenceControlDialog(this);
    zTELOSControl->setModel(walletModel);
    zTELOSControl->exec();
}

void PrivacyDialog::setZTranscendenceControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzTELOS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Transcendence Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if (!ZTranscendenceControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTranscendenceControlDialog::GetSelectedMints();
    }
    ...
    // Clear ztranscendence selector in case it was used
    ZTranscendenceControlDialog::listSelectedMints.clear();
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Transcendence, ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzTELOSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
