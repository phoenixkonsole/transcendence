...
/* *** BEGIN OBFUSCATION MAGIC - TELOSIPCOIN **********
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                adds up to a cost of 0.001 TELOSIPCOIN per transaction on average.
            */
            if (r <= 10) {
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100TELOSIPCOIN+1 ( bit on if present )
    // bit 1 - 10TELOSIPCOIN+1
    // bit 2 - 1TELOSIPCOIN+1
    // bit 3 - .1TELOSIPCOIN+1
    // bit 3 - non-denom
